        ...
        <div className='flex flex-col justify-center items-center p-20'>
            ...
            {todos.map((todo) => (
                <div key={todo.id} className='flex justify-around items-center border-2 w-[60%] h-[50px] mt-3 '>
                    <div className=':text-yellow-300'>{todo.text}</div>
                    <div className='flex gap-3'>
                        <button onClick={()=>handleComplete(todo.id)} className=':text-yellow-300  p-2 rounded-lg'>
                            {todo.completed ? 'Undo' : 'Done'}</button>
                        <button onClick={()=>deleteTodo(todo.id)} className='text-black '>X</button>
                    </div>
                </div>
            ))}
        </div>
